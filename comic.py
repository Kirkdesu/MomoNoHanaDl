...
# Função para processar um manga
def process_manga(manga_url):
    response = requests.get(manga_url)
    soup = BeautifulSoup(response.content, 'html.parser')
    
    # Encontrar todos os links dos capítulos
    chapters = soup.select('ul.main.version-chap.no-volumn li.wp-manga-chapter a')
    chapter_links = [a['href'] for a in chapters]
...
for manga_link in manga_links:
    print(f"Processando manga: {manga_link}")
    process_manga(manga_link)
